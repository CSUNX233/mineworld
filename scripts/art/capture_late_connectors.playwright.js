async page => {
 ...
 const specs=[['abyss-connector-entry',16,'room-0'],['abyss-connector-corridor',19,'room-2'],['abyss-connector-stairs',17,'room-0'],['citadel-connector-entry',21,'room-0'],['citadel-connector-corridor',24,'room-2'],['citadel-connector-stairs',22,'room-0']];
 for(const [name,floor,id] of specs){
  await page.evaluate(async({name,floor,id})=>{
   ...
   const data=generateFloor(411485540,floor,7),base=data.rooms.find(r=>r.id===id),old=data.grid;
   const e=base.entrances[0],r=name.endsWith('stairs')?base:{...base,x:e.x-7,z:e.z-7,width:15,depth:15,center:{x:e.x+.5,z:e.z+.5},entrances:base.entrances,cells:base.cells};

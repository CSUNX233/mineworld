...
for z,w,h in [(.13,.98,.26),(.4,.9,.18),(3.65,.91,.16),(4.42,.98,.23),(4.72,.8,.35),(4.95,.61,.12)]:
    stone(0,0,z,w,w,h)
for side in [-1,1]:
    cube((0,side*.397,3.02),(.35,.025,.64),0,(.61,.59,.51))
    for x in [-.20,.20]:stone(x,side*.42,3.02,.045,.045,.73,1.04)
    for z in [2.66,3.38]:stone(0,side*.42,z,.44,.045,.055,1.04)
    for x,z,w,h in [(0,3.03,.06,.35),(0,3.04,.24,.06),(-.1,3.13,.055,.12),(.1,3.13,.055,.12)]:cube((x,side*.45,z),(w,.025,h),3,(.85,.82,.70))
finish('pillar')
courses(3.2,.9,.9)
...
for y in [-.435,.435]:
    cube((0,y,5.04),(.25,.025,.36),0,(.58,.56,.49))
    stone(0,y*1.06,5.04,.11,.035,.26,1.08)
finish('arch')

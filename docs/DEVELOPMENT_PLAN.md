...
> 本轮执行更新：见 [长期天赋、九件套与死亡收割交付](P6_RELICS_AND_META_DELIVERY.md)。按最新确认，红装/暗金传说暂统一为 mythic 死亡收割九件，现有 Boss 独立 3% 掉落；十二套常规套装新增九件档。局外树扩为 46 节点，完整 25 层胜利三次只解锁后续资格，新体系尚不制作。此约定优先于下文原先红装/暗金分层与获取来源设想。
...
实施记录：2026-09-12，P5机制套装与有限打造已进入试玩验收，见 [P5_DELIVERY.md](P5_DELIVERY.md)。12套采用统一额外效果预算，10%综合差距目标待实战核对；验收前不推进P6。
...
- 同一浏览器内五个存档槽共用局外档案、营地天赋和掌握记录；每槽最多一局独立进行中的游戏。删除单个冒险存档保留共享营地；设置仍由 SettingsManager 全局管理。
...
**最新实施约束：当前继续按 3D 制作全部玩法功能，玩家、敌人和召唤物保留 3D 粗模。HD-2D 角色与场景表现改造安排在全部计划玩法功能完成之后，不提前接入面片角色。用户已选②明亮冒险，并明确提前实施整套 UI 素材生成与接入；这是 UI 范围的提前交付，不启动角色或场景美术替换。** P9 是后续 HD-2D 表现样板阶段；如果 P8 后仍有未完成的玩法功能，先补齐功能再进入 P9。每阶段完成仍停下等待用户验收。UI 素材规范见 [SUNLIT_UI_ASSETS.md](SUNLIT_UI_ASSETS.md)。
...
在全部计划玩法功能完成、尺寸和内容职责稳定后，先制作一套小型 HD-2D 美术样板，再决定是否批量生产。
...
#### 后续 HD-2D 表现方案（功能完成后实施）

- **保留 3D 世界和规则**：地形、建筑、位置、高度、移动、碰撞、寻路、攻击判定与投射物轨迹继续使用 3D。场景以像素贴图、统一光照和模块化建筑提升表现。
- **替换角色表现层**：玩家、敌人和召唤物后续使用带透明背景的 2D 像素动画面片。面片朝向镜头，角色真实朝向由逻辑独立维护；护卫正背面减伤等规则不随面片朝向改变。
- **多方向动画**：以 8 方向作为样板目标，根据镜头相对角色真实朝向选择正面、侧面或背面动画；包含待机、移动、攻击、受击、死亡。统一脚底锚点、尺寸、帧节奏、武器与特效挂点，生成的独立图片不能直接当作已完成的连续动画。
- **镜头兼容**：保留水平转动目标，俯仰范围在样板阶段验证，避免极端视角暴露纸片感。第一人称隐藏自身面片，手和武器采用独立表现；敌人近距离显示另行验收。当前不修改镜头范围或移除第一人称。
- **融入场景**：统一像素尺度、色板、受光、脚底阴影、墙体遮挡与透明边缘处理。技能可结合像素动画面片与 3D 运动；UI 同步统一，文字优先保证桌面与手机可读性。
- **实施顺序**：P9 先做一个可玩房间、8 方向玩家、盾卫、火球和统一 HUD，检查旋转、绕后、攻击时序、遮挡和第一人称；用户验收后 P10 才扩展全部角色、敌人、召唤物和场景资产。
- **当前只预留边界**：功能开发保持渲染与逻辑分离即可，不以未来 HD-2D 为由提前重写渲染器或制作方向动画。五张像素概念图仅用于候选风格比较，具体方向尚未选定，见 [像素风格提案](PIXEL_STYLE_PROPOSALS.md)。

## 7. 定向打造与经济

P5 最新审计与实施设计见 [十二套机制、混搭与打造方案](P5_EQUIPMENT_DESIGN.md)：用户已选择机制与混搭为主，打造及探索为辅，要求至少10套不同玩法并可混搭；现有9套的处理清单与12套目标均已列出。该文档为设计，尚未代表新套装已接入游戏。
...
- **新增约束：流派成熟期的综合战斗表现差距目标不超过 10%。** 比较同等装备预算、天赋投入、玩家熟练度与关卡条件，按最快/最慢清理时间等比值检验；不能用单个技能伤害相同替代整套构筑平衡。
- 允许前期成型快与后期更强的成长曲线差异，分别观察 1–5、6–10、11–20、21–25 层，不要求每个阶段每项指标完全相同。
- 清群、精英、纯 Boss 战必须分开记录，并同时看资源消耗、死亡/受伤和操作负担；禁止用压倒性的保命优势掩盖低伤害或反向包装成等强。
- 混合流派承担双根节点和技能槽机会成本，不可直接叠加两套完整增伤。隐藏的攻速、单位倍率与天赋乘区必须一起审计。
- P4 只完成基础逻辑与明显倍率重叠检查；未完成实际整局对比前，不宣称所有流派已满足 10% 或目标胜率。后续获授权的平衡联测须保持固定策略、装备预算和种子。
...
- 当前阶段：**P4 代码接入完成，等待用户试玩验收**。三基础流派、六个新增技能、混合原型、召唤编队和局外掌握已接入；内容与检查记录见 [P4_DELIVERY.md](P4_DELIVERY.md)。6–10 层内容见 [FOUNDRY_CHAPTER_DELIVERY.md](FOUNDRY_CHAPTER_DELIVERY.md)。完整对局平衡待试玩验证。
...
| P9：统一美术与 HD-2D 样板 | 全部玩法功能完成后，验证 3D 场景与逻辑、多方向像素角色面片、统一 UI 和特效 | 一套实际可玩的 HD-2D 样板，随后补齐代表性表现 | 功能已完成；镜头、方向辨识、遮挡、风格与目标设备表现通过用户验收后再批量制作 |
...
第 16–25 层后续场景与玩法设计见 [章节制作总则](FLOORS_16_25_PRODUCTION_DESIGN.md)、[16–20 蚀光深庭](FLOORS_16_20_DESIGN.md)、[21–25 封界天垒](FLOORS_21_25_DESIGN.md)。围绕现有深渊领主与遗迹监守者展开，每章三种独立交互、六种小怪和五层递进；当前仅完成设计，等待后续分批制作，不代表已接入。

第 6～10 层的新地图、房间和怪物专项设计见 [失控铸造所设计](FLOORS_06_10_DESIGN.md)。用户随后要求连续完成整段，目前 [五层灰盒交付](FOUNDRY_CHAPTER_DELIVERY.md)已接入，等待整体验收。

第 10 层过渡与第 11～15 层的新章节设计见 [沉钟圣陵设计](FLOORS_11_15_DESIGN.md)。用户随后要求连续完成两章，现已接入六种敌人、回响/点名/葬仪与第十五层 Boss，见 [章节交付](CHAPTER_REFRESH_DELIVERY.md)，等待试玩验收。

第 1～5 层丰富度优化见 [石卫遗迹优化文档](FLOORS_01_05_REFRESH.md)。五层子主题、房间变化、补给架、军械挑战与第五层专属 Boss 已接入。最新要求同时上调五章数值压力 5%/10%/15%/20%/25%，旧胜率需重新验证；本轮未跑大规模模拟，现停下等待试玩。

加入治疗技能作为未来构筑目标，本轮不提前实现。至少区分主动瞬间治疗、持续恢复、治疗区域与召唤物辅助治疗，并允许与护盾、异常状态清除和资源消耗形成组合。
治疗强度应与冷却、法力消耗及输出机会成本共同平衡，避免无限续航抹去怪物机制；优先保证施放反馈、范围提示和手机拖动选点清晰。仅当真实治疗发生时显示恢复反馈，不把满血施放表现为有效治疗。


### 2026-09-13：16–25 层首轮制作

场景、新敌人与机关、0.8 米低平台已接入；等待用户验收。见 [实现记录与差异清单](FLOORS_16_25_IMPLEMENTATION.md) 和 [参考 / 实际截图对照](../art/sunlit-world/chapters-16-25-kit/preview.html)。

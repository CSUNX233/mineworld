...
def make_arm(side,s):
    ...
    if side=='R':
        ...
        b.cloth([[(s*.228,-.075,1.421),(s*.256,.005,1.452),(s*.235,.092,1.414)],
                 [(s*.329,-.118,1.383),(s*.365,.005,1.405),(s*.329,.104,1.374)],
                 [(s*.418,-.092,1.298),(s*.443,.005,1.302),(s*.405,.088,1.306)]],
...
def build():
    ...
    scene['revision']='A10 shoulder refinement: relaxed arm rest, tapered sleeves, wrapped ceramic shoulder with visible front lip; sewn short cape'

...
def arm_rest(point):
    """Lower the A-pose 15 degrees to the reference's relaxed shoulder line."""
    x,y,z=point
    sign=1 if x>=0 else -1
    pivot=Vector((sign*.29,0,1.38))
    return pivot+Quaternion(Vector((0,1,0)),sign*math.radians(15))@(Vector(point)-pivot)
    ...
    def cloth(self,rows,weights,tile,thickness=.012):
        ...
        for j in range(height-1):
            for i in range(width-1):
                ...
                self.face([back[j][i+1],back[j+1][i+1],back[j+1][i],back[j][i]],tile,uv[::-1])
        ...
        for k,(j,i) in enumerate(edge):
            ...
            self.face([front[j][i],back[j][i],back[nj][ni],front[nj][ni]],tile)
...
def make_body():
    ...
    # Curved, mostly vertical drape on the left back instead of a diagonal flag.
    cape_rows=[
        [(-.18,.141,1.412),(-.035,.164,1.433),(.155,.177,1.42),(.371,.15,1.416)],
        [(-.142,.184,1.282),(.015,.212,1.29),(.185,.238,1.278),(.341,.184,1.255)],
        [(-.087,.193,1.085),(.058,.24,1.075),(.235,.263,1.055),(.338,.212,1.069)],
        [(-.012,.184,.952),(.105,.22,.84),(.257,.26,.775),(.352,.211,.849)]]
    ...
    b.cloth([[(-.225,.141,1.411),(-.105,.195,1.423),(0,.207,1.407),(.13,.194,1.423),(.27,.15,1.421)],
             [(-.205,.163,1.364),(-.10,.215,1.359),(0,.235,1.322),(.13,.211,1.366),(.25,.173,1.374)]],
            [['chest']*5]*2,0)
    # Visible return of the front baldric and two rear belt keepers.
    b.plate([(-.231,.118,1.372),(-.175,.15,1.381),(-.116,.171,1.049),(-.174,.148,1.049)],-.015,4,'chest')
    b.plate([(-.205,.154,1.278),(-.145,.175,1.278),(-.14,.181,1.237),(-.198,.16,1.237)],-.01,7,'chest')
    b.plate([(-.187,.166,1.268),(-.161,.18,1.268),(-.156,.184,1.247),(-.183,.17,1.247)],-.01,4,'chest')
    for x in [-.138,.133]:
        b.plate([(x-.017,.14,.949),(x+.017,.14,.949),(x+.018,.153,1.041),(x-.018,.153,1.041)],-.018,5,'pelvis')
    # Pouch closure is visible in rear-quarter view, without adding another pouch.
    b.plate([(-.329,.073,.923),(-.216,.075,.925),(-.224,.079,.859),(-.274,.083,.839),(-.321,.077,.862)],-.012,4,'pelvis')
    b.plate([(-.284,.089,.915),(-.261,.089,.915),(-.261,.091,.853),(-.284,.091,.853)],-.008,5,'pelvis')
    # Split back coat tails: enough separation to clear the hip hinge when crouched.
    for side,sign in [('L',1),('R',-1)]:
        b.cloth([[(sign*.018,.144,.951),(sign*.11,.152,.948),(sign*.203,.11,.93)],
                 [(sign*.024,.151,.766),(sign*.119,.171,.753),(sign*.215,.12,.756)],
                 [(sign*.03,.125,.603),(sign*.117,.145,.573),(sign*.188,.104,.642)]],
                [[{'pelvis':.45,'thigh.'+side:.55}]*3,['thigh.'+side]*3,['thigh.'+side]*3],3)
        # Rear boot leather spine provides a readable heel instead of a blank block.
        x=sign*.185
        b.plate([(x-.026,.073,.032),(x+.026,.073,.032),(x+.022,.079,.17),(x-.022,.079,.17)],-.009,4,'foot.'+side)
    return b
...
def make_head():
    ...
    for indexes in [[ids[2][0],ids[2][1],ridge_top],[ids[2][0],ridge_top,ids[2][6]],
        ...
        b.face(indexes,0)
    b.plate([(-.007,.239,1.785),(.007,.239,1.785),(.007,.243,1.613),(-.007,.243,1.613)],-.002,12,'head')
...
def make_arm(side,s):
    ...
    b.tube(points,[(.073,.08),(.101,.097),(.104,.092),(.089,.078),(.065,.06),(.068,.062),(.076,.067),(.052,.049)],2,
...
def socket(scene,rig,name,bone,position,direction=(0,0,1)):
    ...
    direction=Quaternion(Vector((0,1,0)),math.radians(15)*(1 if position[0]>=0 else -1))@Vector(direction)
...
def build():
    ...
    scene['revision']='A12 rear refinement: crowned hood ridge, folded back collar, rear baldric, pouch closure and split coat tails; A11 grounded QA retained'

        ...
        for x in range(256):
            ...
            if t in (13,14,15):
                ...
                if emblem or border:
                    c = rgb[6]
            if t==13:
                # Rear board: block-built asymmetric star, broken checker hem.
                # Dedicated UV layout; no continuous cream border on the cape sides.
                c=rgb[13]
                gx,gy=u//4,v//4
                ...
                hem=(gy<=2 and ((gx+gy)%3!=0)) or (gy==3 and gx in (2,7,12,14))
                if emblem or hem:c=rgb[6]
            i = (y*256+x)*4
...
def make_body():
    ...
    panel=[[(-.081,.143,.956),(0,.155,.956),(.081,.143,.956)],
           [(-.077,.152,.746),(0,.162,.746),(.077,.152,.746)],
           [(-.066,.125,.515),(0,.146,.489),(.066,.125,.515)]]
    b.cloth(panel,[[w]*3]*3,3)
    for side in [0,2]:
        edge=[(row[side][0],row[side][1]+.018,row[side][2]) for row in panel]
        inside=[(x+(.016 if side==0 else -.016),y+.002,z+.004) for x,y,z in edge]
        b.cloth([[edge[i],inside[i]] for i in range(3)],[[w]*2]*3,6,.006)
    b.cloth([[(x,y+.023,z+.035) for x,y,z in panel[-1]],[(x,y+.024,z) for x,y,z in panel[-1]]],[[w]*3]*2,6,.006)
    return b
...
def build():
    ...
    scene['revision']='A13 rear-board dressing and relaxed arms: right cape, block star and checker hem, left rear pouch, plain cream-bordered rear panel; 22 degree arm inset and counter-turned wrists; right weapon and left vessel retained'

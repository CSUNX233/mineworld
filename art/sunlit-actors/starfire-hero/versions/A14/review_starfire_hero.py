...
def pose(name,amount=1):
    ...
    feet_before={s:rig.pose.bones['foot.'+s].matrix.translation.copy() for s in ['L','R']}
    if name in ['flex','crouch','staff']:
        pelvis=rig.pose.bones['pelvis']
        for side in ['L','R']:
            rotate('thigh.'+side,(1,0,0),-48*a)
            rotate('shin.'+side,(1,0,0),88*a)
            rotate('foot.'+side,(1,0,0),-40*a)
        rotate('spine',(1,0,0),26*a)
        rotate('chest',(1,0,0),-3*a)
        rotate('neck',(1,0,0),-12*a)
        rotate('head',(1,0,0),-11*a)
        rotate('tabard',(1,0,0),-50*a)
        rotate('cape.upper',(1,0,0),-8*a)
        rotate('cape.lower',(1,0,0),-8*a)
    if name=='flex':
        # Hands balance forward with relaxed elbows rather than spreading sideways.
        for side,sign in [('R',1),('L',-1)]:
            p=rig.pose.bones['upper_arm.'+side];basis=p.bone.matrix_local.to_quaternion()
            p.rotation_quaternion=basis.inverted() @ (Quaternion(Vector((0,1,0)),math.radians(sign*12*a)) @ Quaternion(Vector((1,0,0)),math.radians(-20*a))) @ basis
            rotate('forearm.'+side,(1,0,0),-48*a)
    if name=='staff':
    ...
    if name=='staff':
        ...
        p=rig.pose.bones['hand.R']; basis=p.bone.matrix_local.to_quaternion()
        p.rotation_quaternion=(basis.inverted() @ Quaternion(Vector((0,1,0)),math.radians(30*a)) @ basis) @ p.rotation_quaternion
        for kind in ['Sword','Staff']:
            o=bpy.data.objects['SF_Proxy_'+kind]; o.hide_set(kind=='Sword'); o.hide_render=kind=='Sword'
    bpy.context.view_layer.update()
    if name in ['flex','crouch','staff']:
        # Grounded QA pose: translate pelvis by the exact ankle displacement.
        # This is authored pose geometry only, never game position/collision logic.
        displacement=sum((feet_before[s]-rig.pose.bones['foot.'+s].matrix.translation for s in ['L','R']),Vector())*.5
        pelvis.location=pelvis.bone.matrix_local.to_3x3().inverted() @ displacement
        bpy.context.view_layer.update()
...
def rig_report():
    ...
    rest_feet={s:rig.pose.bones['foot.'+s].matrix.translation.copy() for s in ['L','R']}
    for name,amount in [('rest',1),('flex',.5),('flex',1),('reach',1),('staff',1)]:
        ...
        foot_error=max((rig.pose.bones['foot.'+s].matrix.translation-rest_feet[s]).length for s in ['L','R'])
        report['inspectionPoses'].append({'name':name,'amount':amount,'nonFiniteMeshes':bad,'gripOriginErrorMeters':error,'plantedAnkleErrorMeters':foot_error})
        if name in ['flex','staff']: assert foot_error<1e-5
...
def inspection_action():
    ...
    for frame in range(1,49):
        phase=(frame-1)/23 if frame<=24 else (48-frame)/24
...
def reimport_check():
    ...
    qa=bpy.data.scenes.new('SF_Export_QA'); qa['starfire_stage_a']=True
    previous=set(bpy.data.objects)
    try:
        bpy.context.window.scene=qa
        bpy.ops.import_scene.gltf(filepath=str(ASSETS/'starfire-hero.glb'))
        bpy.context.view_layer.update()
        meshes=[o for o in qa.objects if o.type=='MESH' and any(m.type=='ARMATURE' for m in o.modifiers)]
        rigs=[o for o in qa.objects if o.type=='ARMATURE']
        points=[o.matrix_world@Vector(c) for o in meshes for c in o.bound_box]
        report={'passed':len(meshes)==4 and len(rigs)==1 and len(rigs[0].data.bones)==30,
                'revision':source['revision'],'skinnedMeshes':len(meshes),'bones':len(rigs[0].data.bones),
                'boundsMin':[min(v[i] for v in points) for i in range(3)],
                'boundsMax':[max(v[i] for v in points) for i in range(3)],
                'importerCustomBoneShapeMeshes':[o.name for o in qa.objects if o.type=='MESH' and o not in meshes],
                'originalSceneObjects':len(bpy.data.scenes['Scene'].objects)}
        assert report['passed']
        (OUT/'blender-reimport-check.json').write_text(json.dumps(report,indent=2),encoding='utf-8')
        bpy.types.blendermcp_server.get_viewport_screenshot(max_size=1600,filepath=str(OUT/'review/final/glb_reimport.png'),format='png')

...
import { combatTexture } from './CombatArt';

/** Reused ground arrow for touch aiming; presentation only, no hit detection. */
export class AimGuide {
  private readonly target = new THREE.Mesh(new THREE.PlaneGeometry(.8, .8), new THREE.MeshBasicMaterial({
    map: combatTexture('telegraphs', 'target'), transparent: true, depthWrite: false, toneMapped: false, side: THREE.DoubleSide,
  }));
  ...
  show(origin: THREE.Vector3, direction: THREE.Vector3, distance: number): void {
    ...
    this.target.position.set(x, y + .01, z);

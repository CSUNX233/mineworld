...
export function buildCampView(envelope: SaveEnvelopeV3, actions: CampActions, message?: string): HTMLDivElement {
  ...
  if (envelope.activeRun) {
    ...
    root.append(paragraph('一大局 25 层，每 5 层一场 Boss 战。第 5、10、15、20 层可提前结算，分别获得 50、100、200、350 研究经验；25 层通关获得 500 研究经验（每 100 经验转化为 1 个局外天赋点）。死亡按所在楼层足额结算，中间楼层按进度插值。主动放弃不发奖励。'));
...
export function buildSettlementView(
  ...
  totals.append(element('strong', `${saved ? '获得' : '保存后获得'} ${record.totalXp} 研究经验 · 转化 ${record.pointsEarned} 个局外天赋点`));
  root.append(totals);
  notice(root, message);

...
const fields = new WeakMap<FloorData, { target: number; distances: Int16Array }>();
export function directionToPlayer(floor: FloorData, x: number, z: number, tx: number, tz: number): {x:number;z:number} | null {
  ...
  if (!field || field.target !== target) {
    ...
    field = {target, distances}; fields.set(floor,field);
  }
  const cx = Math.floor(x), cz = Math.floor(z);
  let best = Infinity, result: {x:number;z:number} | null = null;
  for (const [nx,nz] of [[cx+1,cz],[cx-1,cz],[cx,cz+1],[cx,cz-1]]) {
    if (!isWalkable(floor,nx,nz)) continue;

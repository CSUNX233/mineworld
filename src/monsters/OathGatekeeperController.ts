...
const initial = (): OathGatekeeperState => ({ phase: 1, cooldown: 1.5, cycle: 0, reinforcementUsed: false, angle: 0 });
...
export class OathGatekeeperController {
  ...
  private queue(boss: Monster, duration: number): void {
    ...
    this.warning = new THREE.Mesh(foundrySectorGeometry(this.attackKind === 'shield' ? 2.8 : 3.8, this.state.angle, this.attackKind === 'shield' ? .95 : .8), new THREE.MeshBasicMaterial({
  ...
  update(dt: number, boss: Monster, player: Player, floor: FloorData, host: OathGatekeeperHost, attack: number): void {
    ...
    if (this.state.phase === 2 && !this.state.reinforcementUsed && this.state.cooldown <= 0 && !this.warning) {
      ...
      this.warning = warning; this.scene.add(warning); this.timer = this.duration = 1.4;
    ...
    if (this.warning) {
      ...
      if (distance < (this.attackKind === 'shield' ? 2.8 : 3.8) && Math.cos(angle) >= Math.cos(this.attackKind === 'shield' ? .95 : .8)
      ...
      if (this.state.phase === 2 && !this.second) { this.second = true; this.queue(boss, .85); }
      else { this.second = false; this.state.cooldown = this.state.phase === 1 ? 1.8 : 2.2; }
      return;
    }
    this.state.cooldown = Math.max(0, this.state.cooldown - activeDt);
    ...
    this.state.angle += Math.max(-activeDt * 1.7, Math.min(activeDt * 1.7, difference));
    if (boss.position.distanceTo(player.position) > 3) {
      ...
      this.state.cycle++; this.queue(boss, 1.15);
    }
    boss.faceToward(boss.position.x + Math.cos(this.state.angle), boss.position.z + Math.sin(this.state.angle));
  }
}
